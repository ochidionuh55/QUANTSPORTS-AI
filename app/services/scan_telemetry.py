...
**Observability only, and enforced rather than asserted.** Every write happens
inside its own SAVEPOINT. A failure rolls back only that savepoint, leaving the
scan's transaction usable.

This is not a theoretical precaution. The first deployment shipped a migration
whose timestamp columns had no server default, so the first insert raised an
IntegrityError. The exception was caught — but catching it does not repair a
session, and SQLAlchemy had already marked the transaction unusable. Every
subsequent fixture failed with "transaction has been rolled back" and the scan
analysed nothing. "Best-effort" written in a docstring is not best-effort;
a savepoint is.
...
class ScanTelemetry:
    ...
    async def start(
        ...
        try:
            # Its own savepoint: a failure here must not poison the scan's
            # transaction, which is what happened the first time this shipped.
